...
<div class="fixed inset-y-0 left-0 z-50 w-64 bg-cyan-800 text-white flex flex-col transform lg:translate-x-0"
    ...
    <div class="h-16 flex items-center justify-center px-4 bg-cyan-900 shadow-md">
        <h1 class="text-xl font-bold">DASBOR GURU</h1>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-2 sidebar-scroll">
        ...
        <a href="{{ route('teacher.dashboard') }}"
           class="flex items-center px-2 py-2 text-sm font-medium rounded-md group
                  {{ request()->routeIs('teacher.dashboard') ? 'bg-cyan-900 text-white' : 'text-cyan-100 hover:bg-cyan-700' }}">
        ...
        <div class="space-y-1">
            ...
            <div x-show="akademikMenuOpen" x-transition class="pl-5 space-y-1">
                <a href="{{ route('teacher.dashboard') }}"
                   class="flex items-center px-2 py-2 text-sm font-medium rounded-md group
                          {{ request()->routeIs('teacher.attendances.create') ? 'bg-cyan-900 text-white' : 'text-cyan-100 hover:bg-cyan-700' }}">
                   Ambil Absensi
                   <span class="text-xs text-cyan-300 ml-1">(dari Dasbor)</span>
                </a>
                <a href="{{ route('teacher.attendances.report') }}"
                   class="flex items-center px-2 py-2 text-sm font-medium rounded-md group
                          {{ request()->routeIs('teacher.attendances.report') ? 'bg-cyan-900 text-white' : 'text-cyan-100 hover:bg-cyan-700' }}">
                   Laporan Absensi Saya
                </a>
                {{-- PERUBAHAN DI SINI --}}
                <a href="{{ route('teacher.grades.create') }}"
                   class="flex items-center px-2 py-2 text-sm font-medium rounded-md group
                          {{ request()->routeIs('teacher.grades.*') ? 'bg-cyan-900 text-white' : 'text-cyan-100 hover:bg-cyan-700' }}">
                   Input Nilai
                </a>
            </div>
        </div>
